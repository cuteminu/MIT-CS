...
bisection_times = 0 # The number of bisection search iteratoins. Starting from 0
...
# 방식 잘못됨. 오차 범위 안에 들어올 때까지 iteration 해야함.
...
while diff_cur_savings(start * 0.0001, current_savings, monthly_salary) * diff_cur_savings(end * 0.0001, current_savings, monthly_salary) < 0 :
    mid = (start + end) / 2
    if diff_cur_savings(start * 0.0001, current_savings, monthly_salary) * diff_cur_savings(mid * 0.0001, current_savings, monthly_salary) < 0 :
        end = mid 
    else : 
        start = mid
