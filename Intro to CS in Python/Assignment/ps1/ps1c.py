...
if diff_cur_savings(high*0.0001, current_savings, monthly_salary) < 0 :
    possibility = False

while abs(diff_cur_savings(guess * 0.0001, current_savings, monthly_salary)) >= epsilon and possibility:
    if diff_cur_savings(guess*0.0001, current_savings, monthly_salary) > 0 :
        high = guess
    else :
        low = guess
    guess = (high+low)/2.0
